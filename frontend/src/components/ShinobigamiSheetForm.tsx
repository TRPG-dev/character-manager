...
import { SHINOBI_SCHOOLS, SHINOBI_SKILLS_BY_DOMAIN, getDomainFromSchool, getSkillsByDomain } from '../data/shinobigamiSkills';
...
export const ShinobigamiSheetForm = ({ data, onChange }: ShinobigamiSheetFormProps) => {
  ...
  const [selectedSkillDomain, setSelectedSkillDomain] = useState<string>('体術');
  const [skillTableVisible, setSkillTableVisible] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    ...
    if (normalized.school) {
      setSelectedSkillDomain(getDomainFromSchool(normalized.school));
    }
  }, [data, isInternalUpdate]);

  const updateAttributes = (key: keyof typeof sheetData.attributes, value: number) => {
    const updated = { ...sheetData, attributes: { ...sheetData.attributes, [key]: value } };
  ...
  const updateSchool = (school: string) => {
    ...
    const updated = { ...sheetData, school, skills: sheetData.skills.map(s => ({ ...s })) };
    setIsInternalUpdate(true);
    setSheetData(updated);
    setSelectedSkillDomain(domain);
    onChange(updated);
  };

  const addSkill = (skillName: string, domain: string) => {
    // 既に同じ特技が存在する場合は更新、なければ追加
    const existingIndex = sheetData.skills.findIndex(s => s.name === skillName);
    const newSkills = [...sheetData.skills];
    
    if (existingIndex >= 0) {
      newSkills[existingIndex] = { ...newSkills[existingIndex], domain };
    } else {
      newSkills.push({ name: skillName, value: 0, domain });
    }
  ...
  const toggleSkillTable = (domain: string) => {
    setSkillTableVisible({ ...skillTableVisible, [domain]: !skillTableVisible[domain] });
  };
  ...
  const availableSkills = getSkillsByDomain(selectedSkillDomain);
        ...
        <div>
          ...
          {sheetData.school && (
            <div style={{ marginTop: '0.5rem', padding: '0.5rem', backgroundColor: '#f0f0f0', borderRadius: '4px' }}>
              特技属性: {getDomainFromSchool(sheetData.school)}
            </div>
          )}
        </div>
      </section>
      ...
      <section>
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <select
            ...
            onChange={(e) => setSelectedSkillDomain(e.target.value)}
            style={{
              width: '100%',
              maxWidth: '300px',
              padding: '0.5rem',
          ...
          >
            {Object.keys(SHINOBI_SKILLS_BY_DOMAIN).map((domain) => (
        ...
        <div style={{ marginBottom: '1rem' }}>
          <button
            type="button"
            onClick={() => toggleSkillTable(selectedSkillDomain)}
            style={{
              padding: '0.5rem 1rem',
          ...
          >
            {skillTableVisible[selectedSkillDomain] ? '特技テーブルを閉じる' : '特技テーブルを開く'} ({selectedSkillDomain})
          </button>
          
          {skillTableVisible[selectedSkillDomain] && (
            <div style={{ marginTop: '1rem', border: '1px solid #ddd', borderRadius: '4px', padding: '1rem' }}>
                ...
                {availableSkills.map((skillName) => {
                  ...
                  return (
                    <button
                      key={skillName}
                      type="button"
                      onClick={() => addSkill(skillName, selectedSkillDomain)}
                      style={{
                        padding: '0.5rem',
        ...
        <div style={{ border: '1px solid #ddd', borderRadius: '4px', padding: '1rem' }}>
          <h3 style={{ marginTop: 0, marginBottom: '1rem', fontSize: '1.125rem' }}>選択された特技</h3>
